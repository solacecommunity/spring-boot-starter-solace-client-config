...
description: "Check and update Maven dependencies. Use when upgrading libraries, verifying Spring Boot BOM compatibility, running Dependabot-like checks locally, or planning version upgrades. Validates builds and updates documentation."
---
...
- When upgrading Spring Boot versions
...
- `sol-jcsmp`
- `testcontainers` and `testcontainers-junit-jupiter`
- `gson`
- `okhttp`
- `swagger-annotations` / `swagger-codegen-maven-plugin`
- All Maven plugins with explicit versions (e.g. `central-publishing-maven-plugin`)
...
- Verify compatibility with the current Spring Boot BOM
...
4. If tests pass, run integration tests:
   ```shell
   mvn verify "-Dgpg.skip" > maven_it_tests.log 2>&1
   ```
...
1. Update `<version>` in `pom.xml` (e.g. 3.0.2 → 3.0.3)
...
- If it exists, add a new version entry (e.g. `## 3.0.3 - 2026-04-20`) with a `### Changed` section listing all dependency updates
